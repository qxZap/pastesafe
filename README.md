...
- **House ads** for scrape.land, Penholder, Censory and Vetrosoft: `public/js/showcase.js` and `public/showcase.css` show them as a dismissible bar with one ad at a time under 75rem, and from 75rem as up to three cards stacked in the sticky right rail, which slides up one card at a time when there are more ads than that. The data lives in `public/js/makers.js` with logos in `public/makers/`, instead of `ads.json`: the page CSP (`connect-src 'none'`) blocks fetching JSON, while a JS module loads under `script-src 'self'`. Nothing is named `ads`, because ad blockers block such files and a blocked import would break the page.
